...
public class EvaluationService {
	...
	*/
		...
		String result = phrase.replaceAll("\\B.|\\P{L}", "").toUpperCase(); //regex \\B sets the word boundary to the left and selects
																			// and \\\P{L}all the characters from a whitespace to the boundary
		return result;
	...
	public int getScrabbleScore(String string) {
		...
		{ 
			char[] inputstring = string.toCharArray(); //converts the input string to a CharArray to be comparable
			...
			/*boolean hasSameChar(string){
				  for(value1 : string.toCharArray()){
				    if(string.indexOf(i) < 0 ) return false; //checking to see if string has any of the letters in the value1 array		
			int value = 0;
			if(value1. ) {
				value = 1;
				sum = sum + value;
			}
			if(string.contains('g')    //(i).contains("g"))
			{
				value = 2;
				sum = sum +value;
			}
			if(string.contains("b","c","m","p")){
				value = 3;
				sum  = sum + value;
			}
			if(string.contains("f","h","v","w","y")){
				value = 4;
				sum = sum + value; 
			}
	...
	//exmaples from
    //Matt from SOvF https://stackoverflow.com/questions/18983883/how-to-calculate-scrabble-score-of-a-word
...
*/
	...
	public boolean isArmstrongNumber(int input) {
		// TODO Write an implementation for this method declaration
	  //abcd... = pow(a,n) + pow(b,n) + pow(c,n) + pow(d,n) input string length amount of Integers and raise them whatever number they are 5 to the fifth etc.
...
/*				
		...
		{
		    newGuess[i] = temp.charAt(i) - '0'; //now i have the number passed in a  Integer[] array.
		     newGuess.toString(input);
		}		
		Jon Skeet of SOvF at https://stackoverflow.com/questions/8033550/convert-integer-to-array-of-digits
*/		
//Math.pow(x, y)
...
for(int i = 0; i < cypher.length(); i++ )
{
    ...
    for(int i=0; i < string.length(); i++)
    {    if(string.charAt(i) == string.charAt(i)) //if the 
            count++; 
...
if(count == 26) {
	...
	for(int c = 0; c<set.length; c++)
	{
		...
		/*int MAX = set[0]; 
		static int answer[];
        for (int c = 1; c < i; c++)       //finds the leagest num in the array
            MAX = Math.max(set[c], MAX);
        
        for(int d = 0; d<MAX.length; d++ ) {  // we have the length of however large the array is passed in
               	
        }
        
        int count[] = new int[MAX + 1];   //gives an int arr that is of the size of the passed in set
        answer = new Int[MAX +1];		//makes an array of same size to put answer in
*/        
		...
		for(int c = 0; i < set.length; c++ )
		a = i / set[0];         //finding multples
		b = i / set[1];
		sum = a * (a+1) / 2;   //sum of multples of first digit
		sum1 =b *(b+1) / 2;
		totalsum = sum + sum1;
		return totalsum; 
		
		//if the length of the passed in set is unknown hw
		//only works with sets of 2
*/	
	...
	public boolean isLuhnValid(String string) {
		...
		for (int i =0; i<size; i++)
			array[i] = Integer.parseInt(string[i]); //so array should now be an int array
